...
Narrativa em capítulos: mostrar (hero, serviços) → provar (marcas, portfólio, showreel) → conhecer (Quem Somos?) → avançar (processo + contacto, fundo escuro contínuo). O menu segue esta ordem. Todos os cabeçalhos de secção seguem o mesmo padrão: alinhados à esquerda, etiqueta mono + `.h2` (clamp 40–60 px; 34 px no telemóvel) + subtítulo.
...
5. **Com Quem Trabalhamos?:** 5 painéis que expandem (Restaurantes, Lojas, Hotéis, Empresas, Influencers).
6. **Showreel:** momento-chave do scroll: a imagem entra com a largura da grelha e cantos arredondados e abre até ao ecrã inteiro (`reelOpen`, CSS `animation-timeline: view()`), com link para o Vimeo.
7. **Quem Somos?:** colagem 3D + texto que se preenche com o scroll + números reais do portfólio (16 hotéis, 14 restaurantes, 9 marcas).
8. **Como Trabalhamos:** (início do capítulo final escuro, segue sem corte para o contacto) 4 cartões em escada (01 mais baixo, 04 mais alto; degraus de 24 px).
9. **Contacto:** fundo escuro; à esquerda título "Alavanque a sua marca.", "Fale Connosco." e lista de contactos diretos (email, telefone, Instagram, Vimeo) que fica fixa ao fazer scroll; à direita formulário num cartão, com "O que procura?" em caixas de seleção (7 serviços reais + "Outro", `name="servicos"`, colunas ajustam-se à largura), campos com validação e botão tomate.
10. **Rodapé** compacto: logótipo + links numa linha; © e contactos (email, telefone) na linha de baixo.

"Trabalho Selecionado" e "Outros Projetos" foram removidos por repetirem conteúdo (projetos do portfólio e clientes da faixa de marcas); estão no histórico do git.
...
- **Grelha:** todo o conteúdo alinha pelas mesmas duas linhas verticais. Tokens em `:root` no `css/styles.css`: `--content: 1312px` (largura máxima) e `--gutter` (64 px; 40 px até 1180 px; 20 px até 860 px). Dentro de `.wrap` isto é automático; em elementos a toda a largura (hero, carrossel, moldura do showreel) usar `max(var(--gutter), calc((100% - var(--content)) / 2))`. Não usar paddings laterais fixos em secções novas. **Ritmo vertical:** `--sec-y` (96 px; 80 px até 1180 px; 64 px até 860 px) é o espaço entre o conteúdo de cada secção e o seu limite (`padding: var(--sec-y) 0`); duas secções creme seguidas partilham um só `--sec-y` (`.light + .light{padding-top:0}`); `--head-gap` (40 px; 28 px no telemóvel) é a distância do cabeçalho da secção (etiqueta, título, subtítulo) ao conteúdo. Não usar valores fixos.
...
- As marcas da faixa (`BRANDS`) estão como dados no topo de `js/main.js`.
